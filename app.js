...
function statusToPhase(statusValue) {
  ...
  if (v.includes("transit")) return "Em Trânsito";
...
function normalizeOperacionalName(equipeName) {
  ...
  if (!equipeName || equipeName === "all" || equipeName === "Sem equipe") return "";
...
function buildWebhookUrl({ from, to, operacional } = {}) {
  ...
  // Adiciona parâmetro operacional se uma equipe estiver selecionada
  const operacionalNormalized = normalizeOperacionalName(operacional);
  if (operacionalNormalized) {
...
async function tryLoadFromWebhook({ from, to, operacional, signal } = {}) {
  ...
  function normalizeProcessFromApi(obj) {
    ...
    faseAtual = statusToPhase(faseAtual) || matchPhaseLabel(faseAtual) || "";
...
function getBaseFilteredProcesses() {
  ...
  return processes.filter((p) => {
    const matchesText =
      ...
      norm(p.clienteNome).includes(q) ||
      norm(p.fornecedor).includes(q) ||
      norm(p.responsavel).includes(q) ||
      norm(p.origem).includes(q) ||
      norm(p.modal).includes(q) ||
      norm(p.faseAtual).includes(q);
...
function updateSummary() {
  ...
  for (const p of base) {
    ...
    const status = getTransitoStatus(p);
    if (status === "registrado") transitoRegistrado += 1;
    else if (status === "nao_registrado") transitoNaoRegistrado += 1;
  }
      ...
      <div class="card__top">
        <div class="card__title">
          ...
          <div class="card__sub">
            ...
            ${refLabel ? `<span class="chip"><span class="chip__dot"></span>${escapeHtml(refLabel)}</span>` : ""}
